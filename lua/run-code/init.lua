...
M.run_commands_opt = {
    ...
    haskell = function()
        ...
        return string.format("ghc -O2 -threaded %% -o %s/.tmp_exec && %s/.tmp_exec && rm -f %s/.tmp_exec %%:r.hi %%:r.o", temp, temp, temp)
    end,
...
function M.setup(opts)
    ...
    -- Register commands
    vim.api.nvim_create_user_command("RunCodeDev", function() M.run(false) end, {})
    vim.api.nvim_create_user_command("RunCodeOpt", function() M.run(true) end, {})
...
function M.list_languages()
    local langs = {}
    for k, _ in pairs(M.run_commands_dev) do langs[#langs+1] = k end
    for k, _ in pairs(M.run_commands_opt) do 
        local found = false
        for _, v in ipairs(langs) do if v == k then found = true break end end
        if not found then langs[#langs+1] = k end
    end
    table.sort(langs)
